...
class StatusCommand extends Command {
    ...
    protected function configure() {
        $this
            ...
            ->setDescription('<comment>Displaying</comment> status of time-tracking selected')
            ->addArgument('id', InputArgument::OPTIONAL, 'Displaying time-tracking status by <comment>ID</comment> selected')
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        try {
            ...
            $tracking = $trackingService->getTracking();

            // Output render into terminal
            $output->writeln('<info>[Tracking:<comment>'.$tracking->getId().'</comment>] Display status time-tracking</info>');
